...
func (s *Server) playerGathering(c *gin.Context) {
	...
	stats, err := s.postgres.GetPlayerGatheringStats(postgres.Region(server), playerID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to fetch gathering stats"})
		return
	}
